...
6. **No Need for Web Development Skills**: With Shiny, you can create web apps using R code alone. Knowledge of HTML, CSS, or JavaScript is not necessary, though it can help you to create more custom apps.
7. **Reactivity**: Shiny's reactivity system is excellent. It makes it possible to create applications that automatically update outputs in response to changes in inputs.
...
* Data is on the level of `85` regions (`N = 85`)
* `geometry`: variable that describes the geographic shape of regions (at time we'll have to exclude this column)
...
3. Create an rscript file that you call `app.R` and save the script.
...
### Coding practices
- Use document outline for navigation: Cntrl + Shift + O
  - Number navigation for large apps
