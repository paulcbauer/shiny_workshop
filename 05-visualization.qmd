...
1.  Create a tab item called "model"
2.  Create an initial layout containing a fluid row with one column and one box
3.  Create a plot output widget

-   The newly created tab Item has the tab name `model`

-   We already created another tab item called `intro`, so we can tell where our new tab item goes:
...
dashboardPage(
  ...
  sidebar = dashboardSidebar(                             # <1>
    ...
    sidebarMenu(                                          # <1>
      id = "sidebarMenu",                                 # <1>
      menuItem(tabName = "intro", text = "Introduction"), # <1>
      menuItem(tabName = "model", text = "Model data")    # <1>
    )
  ),
  body = dashboardBody(                                   # <2>
    tabItems(                                             # <2>
      tabItem(tabName = "intro"),                         # <2>
      model                                               # <2>
    )
  )
)
...
2.  Create the tab items within the body. The function `tabItems()` contains all tab objects. We add our newly created `model` object after the introduction. Again, the order and names of `tabItem()`s corresponds to the order and names of `menuItem()`s!
	...
	fluidRow(                                                             # <1>
      ...
      box(                                                              # <1>
				...
				selectizeInput(                                                 # <3>
					...
					choices = setNames(variables, sapply(txts, "[[", "title")),   # <3>
					multiple = TRUE,                                              # <3>
					selected = "Commerce"                                         # <3>
				),                                                              # <3>
				checkboxInput(                                                  # <4>
...
1.  Create a reactive expression that takes care of data cleaning and stores the cleaned data in a reactive object called `dt`
2.  Execute the reactive expression (and thus update `dt`), if and only if the refresh button is pressed
3.  Create a pairs plot using the newly created `dt()` object. This is the same as the `dt` dataframe that we used before with the difference that `dt()` updates every time `input$model_x`, `input$model_y` or `input$model_std` are changed.
...
1.  We renamed the `dt()` reactive object to `mparams()` as it now holds multiple arguments instead of just one dataframe
...
```{r eval=FALSE}
#| code-fold: true
#| code-summary: Full code for comparable visualization
...
Currently, we have three input widgets to change the appearance of plots: `model_x`, `model_y`, and `model_std`. Implement another input widgets that allows users to manipulate the data, output or the plot appearance.
:::
