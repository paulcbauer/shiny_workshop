...
- `output`: a list-like object containing outputs named according to `output ID`
  ...
  - Q: How many outputs are there below?
...
- Below we explore them using examples of simple apps (we'll run them together)
  - You can also use `runApp("appname", display.mode = "showcase")`
...
### reactive()
- `reactive()`: used to create a reactive expression, which is a piece of code that can access reactive values (like inputs), do something with them, and return a value. 
...
```{r eval=FALSE}
## install.packages("pacman")
pacman::p_load(shiny, htmltools, bs4Dash, fresh,
               waiter, shinyWidgets, Guerry, sf,
               tidyr, tidyverse, RColorBrewer,
               viridis, leaflet, plotly, jsonlite,
               GGally, datawizard, parameters,
               performance, ggdark, modelsummary)
# 1 Data preparation ----
...
data_guerry_tabulate <- data_guerry %>% 
  st_drop_geometry() %>% 
  mutate(across(.cols = all_of(unlist(variable_names)), round, 2))
...
## Guerry app: Tabulated data (download)
- Below we will extend our app above to add saving options for the tabulated data in different formats.


```{r 04-server-reactive-programming-13-23, cache=TRUE, eval=FALSE, echo=TRUE}
## install.packages("pacman")
pacman::p_load(shiny, htmltools, bs4Dash, fresh,
               waiter, shinyWidgets, Guerry, sf,
               tidyr, tidyverse, RColorBrewer,
