...
- `output`: a list-like object containing outputs named according to `output ID`
  ...
  - Q: How many inputs/outputs are there in the code below?
...
Start by deciding **how many** and **which inputs (1)**, **reactives (2)** and **ouputs (3)** there are. Then start drawing with inputs representing first column on the left. You could use, e.g., `a>` for inputs, `>c>` for reactives and `>f` for outputs and arrows to connect them.
:::
