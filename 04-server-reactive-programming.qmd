...
## Reactive programming (1) {#sec-name-app}
- ONE HEADLINE FOR REACTIVE PROGRAMMING
...
- Below we add one to the app in @sec-name-app above:
...
- Sometimes good require users to opt-in to performing expensive calculation by requiring them to click an `actionButton()`
  - Here we need `eventReactive()`: first argument specifies what to take a dependency on, second argument specifies what to compute
  - Allows app to only compute certain things when `actionButton()` is clicked
  - See code example in [Chapter[3.5.2](https://mastering-shiny.org/basic-reactivity.html#on-click)  
- See [Chapter 3.5](https://mastering-shiny.org/basic-reactivity.html#controlling-timing-of-evaluation) for more background on `eventReactive()` and `reactiveTimer()`.


## Observers
- So far focus on what's happening inside the app
- Sometimes reactivity needs to extend outside the app
...
- **observer** using `observeEvent()` function is the solution to such problems
...
- Example [here](https://mastering-shiny.org/basic-reactivity.html#observers) illustrates sending message to console

## Binding events
https://shiny.posit.co/r/reference/shiny/1.6.0/bindevent
- EXPLAIN DIFFERENCE bind_events event reactive, isolate (shortly)
- isolate() will be discussed later

## Reactivity in our app
* Below we have the basic code underlying our app. How does reactivity work in this app and how would the reactive graph look like?


```{r 04-server-reactive-programming-11, cache=TRUE, eval=FALSE, include=TRUE, echo=FALSE}
library(shiny)
library(Guerry)
library(tidyverse)
library(sf)
...
- How to free memory
- Add input again... (variable selection)
```{r 04-server-reactive-programming-12, cache=TRUE, eval=FALSE, include=TRUE, echo=FALSE}
library(shiny)
library(reactlog)
