...
- `input`: a list-like object that contains all the **input data sent from the browser**, named according to the **input ID**
  ...
  - `input` is selective about who is allowed to read from it
    - can only be read from within reactive contexts created by a **reactive functions** like `renderText()` or `reactive()`
...
server <- function(input, output, session) {
  output$greeting <- renderText("Hello human!") # <1>
}
...
- **Common coding errors**: Forgetting render function or reading from outputs (see [here](https://mastering-shiny.org/basic-reactivity.html#output))
...
- Q: What do you think are the following render functions used for?
  - `renderImage({...})`
  - `renderPlot({...})`
  - `renderPrint({...})`
  - `renderTable({...})`
...
- **Avoid duplication**: Below example of two outputs relying on same reactive expression `string` (avoid duplication!)
...
2.  Draw the reactive graph for the following three server functions (what are the inputs, reactives and ouputs):

::: {.callout-tip collapse="true"}
Start by deciding **how many** and **which inputs**, **reactives** and **ouputs** there are. Then start drawing with inputs representing first column on the left. You could use, e.g., `a>` for inputs, `>c>` for reactives and `>f` for outputs and arrows to connect them.
:::
...
- Helpful vocabulary (see @fig-prod-consumer)
...
- Shiny provides a variety of reactive functions such as `reactive()`, `observeEvent()`,  `eventReactive()` and `bindevent()`
- Below we explore them using a simply example. You can run those app within R.
- See discussions on reactivity and reactive functions in [Chapter[3.5.1](https://mastering-shiny.org/basic-reactivity.html#timed-invalidation)


### reactive()
* `reactive()`: used to create a reactive expression, which is a piece of code that can access reactive values (like inputs), do something with them, and return a value. A reactive expression is "reactive" in the sense that if its dependencies change, it will automatically update.
...
* `observe()`: creates an observer that runs whenever any of its reactive dependencies change, i.e., code inside `observe()` will be re-evaluated whenever any reactive inputs or reactive expressions that it references get updated.
* `observeEvent()` (observers): used when you want to perform an action in response to an event, but you don't need the result of the action to be used in the UI.


::: {.callout-note collapse="true"}
In this Shiny app, when the "Generate Random Number" button is clicked, a random number is generated and displayed. The number changes each time you click the button. The observeEvent() function is used to perform an action (generating and displaying the random number) when the button is clicked.
:::
...
# Define UI
ui <- fluidPage(
    actionButton("button", "Generate Random Number"),
...
server <- function(input, output) {
    observeEvent(input$button, {
        output$randomNumber <- renderText(runif(1)) # Generate a random number when button is clicked
    })
}
...
server <- function(input, output) {
    ...
    bindEvent(input$dispButton, output$randomNumber) # Binding the output$randomNumber reactive expression to dispButton
}
...
* Below the basic code underlying the Tabulate tab of our app.
  - What would the reactive graph look like for this app?
  - Which reactive functions can you identify?


```{r 04-server-reactive-programming-13, cache=TRUE, eval=FALSE, echo=TRUE}
library(shiny)
library(Guerry)
library(tidyverse)
library(sf)
...
## Guerry app: Aggregation & saving of tabulated data
* Add code here for tabulate data in which data is aggregated and can be saved
