...
2.  Using the same symbols as in @fig-graph-expression, please draw the reactive graphs for the following three server functions. Always start by making a list of all the contained inputs, reactives and ouputs.
...
2. ...draw the reactive graph.

* `xvar>` &rarr; `>filtered>`
* `yvar>` &rarr; `>filtered>`
* `countries>` &rarr; `>filtered>`
* `range>` &rarr; `>filtered>`
