...
```{r eval=FALSE} st_geometry(guerry)}
```
  ...
  fluidRow(                    # <2>
    column(                    # <2>
      ...
      box(                     # <2>
        ...
        collapsible = FALSE,   # <2>
        width = 12,            # <2>
        leaflet::leafletOutput("exp_map", height = "800px", width = "100%") # <3>
      ) # end box
    ) # end column
  ) # end fluidRow
) # end tabItem
...
- In the server function, we have a lot to do (annotated with their respective letters in the code below):
    - Apply the selected aggregation level (A)
    - Apply the selected palette (B)
    - Change hard code to adaptive code (C)
...
1.  Initialize an isolated output widget (i.e., no dependencies) / `output$exp_map`
