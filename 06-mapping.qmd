...
-   How do we work with CRS in R?
    ...
    -   Assign a CRS to an existing sf dataframe: [`st_crs`](https://r-spatial.github.io/sf/reference/st_crs.html)`(...) <- 4326` or `st_set_crs(…, 4326)`
...
-   Let us take a look back at `guerry` and how to handle it as a spatial dataframe
...
-   Spatial operations like spatial aggregations are one of the main differences between spatial and regular data
  ...
  fluidRow(                    # <2>
    column(                    # <2>
      ...
      box(                     # <2>
        ...
        collapsible = FALSE,   # <2>
        width = 12,            # <2>
        leaflet::leafletOutput("exp_map", height = "800px", width = "100%") # <2>
      ) # end box
    ) # end column
  ) # end fluidRow
) # end tabItem
...
```{r eval=FALSE}
output$exp_map <- leaflet::renderLeaflet({                                   # <1>
		leaflet() %>%                                                            # <2>
...
1.  Fill the output `exp_map` with a leaflet map using `renderLeaflet`
...
params <- reactive({
	values <- ~Literacy                                    # <1>
	pal <- colorNumeric(palette = "Reds", domain = NULL)   # <2>
...
output$exp_map <- leaflet::renderLeaflet({
	params <- params()
	leaflet(data = st_transform(guerry, 4326)) %>%        # <3>
		addProviderTiles("OpenStreetMap.France") %>%        # <4>
		setView(lng = 3, lat = 47, zoom = 5) %>%            # <5>
		addPolygons(                                        # <6>
			fillColor = ~params$pal(Literacy),                # <7>
			fillOpacity = 0.7,                                # <8>
			weight = 1,                                       # <8>
			color = "black",                                  # <8>
			opacity = 0.5,                                    # <8>
			highlightOptions = highlightOptions(              # <9>
				weight = 2,                                     # <9>
				color = "black",                                # <9>
				opacity = 0.5,                                  # <9>
				fillOpacity = 1,                                # <9>
		...
		addLegend(                                          # <10>
			...
			pal = params$pal,                                 # <11>
			values = params$values,                           # <11>
			opacity = 0.9,
			title = "Literacy",
			labFormat = labelFormat(suffix = " %")            # <12>
		)
})
```

1.  Define the column of the spatial dataset that should be visualized. Note that columns are passed as formulas, not as vectors.
2.  Determine the color palette for mapping. Here, we want to map continious values in red. The output is a function called `pal()` which we can use later on.
3.  Create an empty map. Add spatial data. Note that leaflet by default only accepts spatial data with EPSG:4326. For anything else, consult [`leaflet::leafletCRS()`](https://rstudio.github.io/leaflet/projections.html), but don't expect to understand much of what's going on.
4. Add a basemap of OpenStreetMap France
5. Set the center and zoom level of the initial view
6. The `addPolygons()` function adds polygons to the leaflet map
7. `fillColor` determines how the polygons should be colored. We pass a formula that contains a call to our generated function `pal()` (see point 2) that maps colors to the Literacy variable.
8. We can add additional parameters that control the appearance of the map, e.g. opacity, color or line thickness (weight).
9. `highlightOptions()` allows you to add a nice highlight effect when hovering over the polygons
10. Using `addLegend()` we add a legend to the bottom right of the Leaflet map
11. `addLegend()` accepts values in the same way as `addPolygons()`: `pal` accepts the generated palette function and `values` accepts a formula containing the column name in the input dataset.
12. Finally, `labelFormat()` allows you to change the formatting of the legend label, e.g. if you need to specify whether a number is a percentage, meters or something else
		...
		column(                                     # <1>
			...
			box(                                      # <1>
				...
				selectInput(                            # <2>
					...
					choices = setNames(variables, sapply(txts, "[[", "title")) # <2>
				),                                      # <2>
				uiOutput("exp_desc")                    # <2>
			),
			box(                                      # <3>
...
```{r eval=FALSE}
  observe({ # <1>
    params <- params() # <2>
    leafletProxy("exp_map", data = params$poly) %>% # <3>
      clearShapes() %>% # <4>
      clearControls() %>% # <4>
      addPolygons( # <5>
      ...
      addLegend( # <5>
        ...
        opacity = 0.9, # <5>
        title = txts[[params$var]]$lgd, # <5>
        labFormat = labelFormat(suffix = txts[[params$var]]$unit) # <5>
      ) # <5>
  }) # <5>
```
...
```{r eval=FALSE}
observe({
	click <- input$exp_map_shape_click # <1>
	req(click) # <2>
	leafletProxy("map_exp") %>% # <3>
		clearMarkers() %>% # <4>
		addMarkers(lng = click$lng, lat = click$lat) # <5>
})
```

1. Store event object. The event is invalidated when a user clicks on a shape in the `exp_map` leaflet widget
...
Change the icon of the marker from exercise 6 to an icon depicting city and the color to red.
:::
